...
func InitWithYamlFile(filePath string) error {
	...
	return Init(config.ClusterNodes, config.MaxConcurrency)
}
...
func (c *Client) Query(ctx context.Context, sql string, params *Params, opts *QueryOptions) (*Records, error) {
	...
	resp, err := c.executor.Request(ctx, c.dbName, ep_QUERY, http.MethodPost, headers, body, timeoutSec, 2)
	if err != nil {
		return nil, err
	}
	defer resp.Release()
...
func (c *Client) Execute(ctx context.Context, sql string, params *Params) (*ExecuteResult, error) {
	...
	resp, err := c.executor.Request(ctx, c.dbName, ep_EXECUTE, http.MethodPost, headers, body, 0, 2)
	if err != nil {
		return nil, err
	}
	defer resp.Release()
